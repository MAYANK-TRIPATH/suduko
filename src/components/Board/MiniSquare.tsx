...
type MiniSquareProps = {
  row: number;
  col: number;
};

const MiniSquare: React.FC<MiniSquareProps> = ({ row, col }) => {
  ...
  const isSelected = () => {
    ...
    if (selectedCell?.cell) {
      selectedCell.squares.forEach((sq: [number, number]) => {
  ...
  return (
    <div
      ...
      } ${isSelected().other ? "bg-slate-900" : ""} rounded-md hover:outline outline-[1px] relative`}
    >
      {Qboard[row][col].value !== 0 && (
        <span
          className={` ${
